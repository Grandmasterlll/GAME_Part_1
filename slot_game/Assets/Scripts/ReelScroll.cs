...
public class ReelScroll : MonoBehaviour
{
    ...
    private void linear(RectTransform reels)
    {
        reelDictionary[reels].ReelState = ReelState.Spin;
        DOTween.Kill(reels);
        reels.DOAnchorPosY(linearDist, linearD).SetEase(Ease.Linear).OnComplete(() => stop(reels));
    }
    private void stop(RectTransform reels)
    {
    ...
    private void NextReel(RectTransform reels)
    {
        ...
        reelDictionary[reels].ResetSPos(curentreelpos);
